...
(function () {
  ...
  const API = {
    ...
    /** 保存 MinerU 配置（业务代码统一走 window.API，不直写 localStorage） */
    ...
    async ocrRecognize(image, opts = {}) {
      ...
      if (this.mode === "remote") {
        ...
        const t0 = Date.now();
        // 服务端单任务最长约 8 分钟（extract 240s + flash-extract 240s），轮询上限放宽到 9 分钟
        ...
        throw new Error("OCR 任务超时（>9 分钟），请稍后重试");
      }
